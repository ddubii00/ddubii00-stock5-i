...
- 5줄 요약 앞에는 제목, 인사말, 설명 문장을 절대 쓰지 마라.
- 답변의 첫 줄은 반드시 5점 만점 종합 점수로 시작한다. 예: "- 종합 점수: 3.5/5"
...
- 종합 판단 표에는 "최종 등급", "등급 의미", "판단 근거" 항목을 포함하라.
...
- 결과는 5줄 요약, 상세 설명, 표 형식으로 출력하라.
- JSON 형식 분석 결과는 출력하지 마라.`;
...
async function requestGemini({ apiKey, model, contents }) {
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
    ...
    body: JSON.stringify({
      system_instruction: {
        parts: [{
          text: '사용자가 제공한 차트 이미지에 보이는 정보만 근거로 기술적 분석을 작성한다. 투자 자문처럼 단정하지 말고 조건부 시나리오로 답한다. 답변이 길어도 5줄 요약, 상세 설명, 표를 끝까지 완성한다. JSON 형식은 출력하지 않는다.',
        }],
      },
      contents,
      generationConfig: {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    contents = [
      ...
      { role: 'model', parts: [{ text: resultParts.join('\n\n') }] },
      {
        role: 'user',
        parts: [{
          text: '분석 결과가 출력 제한으로 중간에 끊겼다. 앞 내용은 반복하지 말고 바로 이어서 남은 상세 분석과 표를 끝까지 완성하라. JSON 형식은 출력하지 마라.',
        }],
      },
    ];
  }
